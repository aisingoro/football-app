			...
			<div class="Purchase-p">
				<div class="Purchase-list">
					<div class="Purchase-item-list"
						...
						<div class="Purchase-money">{{Number(items.rechargeamount)/100}}元<br>
...
export default {
  ...
  methods:{
    ...
    //请求账户详情列表（分页）
    getList(page,type){
      https.fetchPost('/user/rechargelist.jsp',{page} ).then((data) => {
        ...
        this.isDisableScroll = false
          ...
          data.data.list[j].showInfoItem=false
					 switch(data.data.list[j].rechargetype){
						case '0001':
							data.data.list[j].rechargetype='内参'
						break;
						case '0002':
							data.data.list[j].rechargetype='预测'
						break;
						case '0003':
							data.data.list[j].rechargetype='充值'
						break;
						case '0004':
							data.data.list[j].rechargetype='内参包月'
						break;
						case '0005':
							data.data.list[j].rechargetype='预测包月'
						break;
						case '0090':
							data.data.list[j].rechargetype='签到'
						break;
						case '0097':
							data.data.list[j].rechargetype='卖出预测提现预先扣除'
						break;
						case '0099':
							data.data.list[j].rechargetype='不中退款'
						break;
						case '0098':
							data.data.list[j].rechargetype='提现'
						break;
						case '0006':
							data.data.list[j].rechargetype='购买比分'
						break;
						case '0007':
							data.data.list[j].rechargetype='购买半全场'
						break;
          }
				}
